...
mvn -f ../../pom.xml exec:exec -Dexec.executable="java" -Dexec.args="%J_OPTS% -cp %%classpath net.sf.saxon.Transform -init:com.nkutsche.xslt.pkg.handler.PackageManager -xsl:%~dp0\demo.xsl -it -s:%~dp0\xpaths.xml -o:%~dp0\results/demo.txt %xpath%"
