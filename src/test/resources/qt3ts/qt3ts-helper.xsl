...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- <xsl:function name="xpf:format-number" xmlns:xpf="http://www.nkutsche.com/xmlml/xpath-engine/functions" 
    ...
    <xsl:function name="xpmt:env-uri-resolver" as="element(mlml:document)?">
        <xsl:param name="relative" as="xs:string?"/>
        <xsl:param name="baseUri" as="xs:string"/>
        ...
        <xsl:try>
            ...
            <xsl:catch errors="mlml:xml-syntax-error">
                <xsl:sequence select="error(xs:QName('err:FODC0002'), $err:description)"/>
            </xsl:catch>
        </xsl:try>
    </xsl:function>
    ...
    <xsl:function name="xpmt:attach-base-uri" as="node()">
    ...
    <xsl:template match="qt:param[@name][@select]" mode="xpmt:execution-context">
        ...
        <xsl:map-entry key="QName($ns-uri, $name)">
            <xsl:sequence select="mlmlp:xpath-evaluate((), @select, map{'namespaces' : $predef-ns})"/>
<!--            <xsl:evaluate xpath="@select" namespace-context="$predef-nscontext-for-saxon"/>-->
    ...
    <xsl:template match="qt:assert-string-value[@normalize-space = 'true']" mode="xpmt:result-compare" as="xs:boolean">
        ...
        <xsl:variable name="result" select="
            ...
            then mlmlp:xpath-evaluate((), '$result ! string(.)', map{
                    'variable-context' : map{QName('', 'result') : $result},
                    'namespaces' : $predef-ns
                }) 
            else $result
    ...
    <xsl:template match="qt:assert-type" mode="xpmt:result-compare" as="xs:boolean">
        ...
        <xsl:sequence select="mlmlp:xpath-evaluate((), $xpath, $context)"/>
    ...
    <xsl:template match="qt:assert-xml[@file]" mode="xpmt:result-compare" priority="10" as="xs:boolean">
        <xsl:variable name="base-uri" select="resolve-uri(@file, base-uri(.))"/>
        <xsl:variable name="text" select="unparsed-text($base-uri)"/>
        <xsl:next-match>
<!--            <xsl:with-param name="xml" select="mlml:parse-xml-fragment($text, $base-uri)"/>-->
            <xsl:with-param name="expected-xml" select="parse-xml-fragment($text)"/>
        </xsl:next-match>
    </xsl:template>
    
    <xsl:template match="qt:assert-xml" mode="xpmt:result-compare" as="xs:boolean">
        <xsl:param name="result" as="item()*" tunnel="yes"/>
<!--        <xsl:param name="xml" select="mlml:parse-xml-fragment(replace(., '^\s+|\s+$', ''), base-uri(.))" as="node()*"/>-->
        <xsl:param name="expected-xml" select="parse-xml-fragment(replace(., '^\s+|\s+$', ''))" as="node()*"/>
        
        <xsl:sequence select="
            if ($result instance of element(mlml:document)) 
            then xpmt:xml-equal(mlml:doc($result), $expected-xml, @ignore-prefixes = 'true') 
            else if ($result instance of element()*) 
            then xpmt:xml-equal($result ! mlml:as-node(.), $expected-xml/node(), @ignore-prefixes = 'true') 
            else false()
            "/>
